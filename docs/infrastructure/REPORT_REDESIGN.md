...
## Inventario de reportes en DEV (47 QWeb)

Cliente / operación (rediseñados vía layout + CSS + herencias):
...
- RFQ / orden de compra
- Recibo de pago
- Estado de cuenta (nuevo, `res.partner`)
- Garantía (`justech.warranty`)
- Delivery / picking / return slip (CSS + extras; no hay pickings de prueba en DEV)

No se rebindan acciones oficiales (`justech_report_identity_guard`).

Etiquetas, badges HR, reportes internos de márgenes: fuera de documentos comerciales A4; no se rediseñaron como facturas.
...
No se copió Hellenia. Se reutilizó: NCF/RNC, banco por `company_id`, firma en blanco, core tables.

## Arquitectura

`web.external_layout` → `justech_alexander_reports.external_layout_doralex`
+ clase `dx-theme-{DOR|PIN|DOM|MAY|REM|BLU}`
+ CSS modular
+ herencias no destructivas
+ `_render_qweb_pdf` fuerza `with_company(document.company_id)`
...
BLACK_WHITE_READABILITY = PASS (totales también en peso/tamaño, no solo color)
...
READY_FOR_REPORTS_PRODUCTION = NO hasta NCF reales, términos cargados y pickings de inventario de prueba.
