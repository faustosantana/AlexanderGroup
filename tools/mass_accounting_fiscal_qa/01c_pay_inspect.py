...
print(
    ...
    p.amount,
    p.move_id,
    getattr(p, "outstanding_account_id", None),
)
print(
    "PAY_FIELDS",
    [
        f
        for f in p._fields
        if "outstanding" in f or "move" in f or "state" in f or "reconcile" in f
    ],
)
inv = env["account.move"].search(
    [("ref", "like", "DXQA-MASS-20260831-C11-S-FULL01")], limit=1
)
...
print(
    "journal fields",
    [f for f in j._fields if "outstanding" in f or "suspense" in f or "payment" in f],
)
for f in j._fields:
